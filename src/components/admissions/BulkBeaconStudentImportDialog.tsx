...
import {
  Upload, FileText, Loader2, CheckCircle, XCircle, Download, AlertTriangle, School,
} from "lucide-react";
...
interface ParsedRow {
  ...
  dob: string;
  gender: string;
  grade: string;                  // raw grade string from CSV
  section: string;
  admission_no: string;           // existing SchoolKnot admission no
  father_name: string;
  father_phone: string;
  mother_name: string;
  mother_phone: string;
  fee_type: string;               // 'new' | 'existing' | '' (auto)
  // Resolved
  course_id: string | null;
  fee_version: "new_admission" | "existing_parent";
  // Status
  valid: boolean;
  error: string;
}

interface BulkBeaconStudentImportDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess: () => void;
}

// Maps grade name variants → course code suffix
...
function resolveGrade(raw: string, courses: Course[], campus: Campus | null): string | null {
  const key = raw.trim().toLowerCase();
  const suffix = GRADE_MAP[key];
  if (!suffix) return null;

  // Prefer campus-specific prefix: Avantika → BSAV, Arthala → BSA
  const prefix = campus?.name?.toLowerCase().includes("avantika") ? "BSAV"
    : campus?.name?.toLowerCase().includes("arthala") ? "BSA"
    : null;

  if (prefix) {
    const match = courses.find(c => c.code === `${prefix}-${suffix}`);
    if (match) return match.id;
  }
  // Fallback: first match with suffix
  const fallback = courses.find(c => c.code.endsWith(`-${suffix}`));
...
export function BulkBeaconStudentImportDialog({ open, onOpenChange, onSuccess }: BulkBeaconStudentImportDialogProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase.from("courses").select("id, name, code").or("code.like.BSAV-%,code.like.BSA-%").order("code"),
      supabase.from("campuses").select("id, name").or("name.ilike.%beacon%,name.ilike.%avantika%,name.ilike.%arthala%"),
      supabase.from("admission_sessions").select("id, name").eq("is_active", true),
    ]).then(([c, cam, s]) => {
      if (c.data) setCourses(c.data);
      if (cam.data) { setCampuses(cam.data); if (cam.data.length === 1) setSelectedCampusId(cam.data[0].id); }
      if (s.data) { setSessions(s.data); if (s.data.length === 1) setSelectedSessionId(s.data[0].id); }
    });
  }, [open]);
  ...
  const reparse = (rows: ParsedRow[], forceExisting: boolean) =>
    rows.map(r => ({
      ...r,
      fee_version: (forceExisting || r.admission_no.trim() || r.fee_type === "existing")
  ...
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      const data: ParsedRow[] = rows.slice(1).map(cols => {
        ...
        const course_id = grade ? resolveGrade(grade, courses, selectedCampus) : null;

        let valid = true, error = "";
        if (!name) { valid = false; error = "Name required"; }
        else if (grade && !course_id) { valid = false; error = `Unknown grade: "${grade}"`; }

        const fee_version: "new_admission" | "existing_parent" =
          (applyExistingFeeToAll || admission_no || fee_type === "existing")
            ? "existing_parent" : "new_admission";

        return { name, dob, gender, grade, section, admission_no, father_name, father_phone,
          mother_name, mother_phone, fee_type, course_id, fee_version, valid, error };
      });
  ...
  // Recompute fee versions when toggle changes
  useEffect(() => {
    if (parsed.length) setParsed(prev => reparse(prev, applyExistingFeeToAll));
  }, [applyExistingFeeToAll]);

  const handleImport = async () => {
    const valid = parsed.filter(r => r.valid);
    if (!valid.length || !selectedCampusId || !selectedSessionId) return;
    setImporting(true);
  ...
  const downloadTemplate = () => {
    const header = "name,dob,gender,grade,section,admission_no,father_name,father_phone,mother_name,mother_phone,fee_type";
    const example = "Rohan Sharma,2016-06-12,Male,Grade III,A,1803188,Rajesh Sharma,9876543210,Priya Sharma,9876543211,existing";
    const example2 = "Ananya Singh,2017-09-01,Female,Grade II,B,,Amit Singh,9876543212,Sunita Singh,9876543213,new";
    const csv = [header, example, example2].join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = "beacon_student_import_template.csv"; a.click();
  ...
  const canImport = validCount > 0 && !!selectedCampusId && !!selectedSessionId;
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <School className="h-5 w-5" /> Bulk Import Beacon Students
          </DialogTitle>
        </DialogHeader>
        ...
        {result ? (
          ...
          <div className="space-y-4 mt-2">
            {/* Campus + Session selectors (always visible) */}
                  ...
                  <table className="w-full text-xs">
                    <thead className="sticky top-0">
                      <tr className="border-b border-border bg-muted/60">
                        ...
                        <th className="px-3 py-2 text-left font-semibold text-muted-foreground">Admission No.</th>
                        <th className="px-3 py-2 text-left font-semibold text-muted-foreground">Fee Structure</th>
