...
export function ActionBucketSection({
  ...
  return (
    <div className={`rounded-xl border ${color} border-l-4 bg-card overflow-hidden`}>
      {/* Header */}
      <button
        className={`w-full flex items-center gap-2 px-4 py-3 ${headerBg} hover:brightness-95 dark:hover:brightness-110 transition-all`}
      ...
      >
        ...
        <span className="ml-auto flex h-6 min-w-[24px] items-center justify-center rounded-full bg-foreground/10 px-2 text-xs font-bold text-foreground">
          {leads.length}
        </span>
      </button>
