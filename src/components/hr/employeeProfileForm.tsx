// The employee record: loading, editing and saving it.
//
// Extracted from EmployeeProfileDialog so the same forms back both the modal and
// the full-page /employee/:id view. Everything stateful lives in the hook; the
// tabs are a dumb renderer over it. Each surface brings its own header, which is
// the only part that genuinely differs — a modal wants a title bar, a page wants
// Keka's cover strip.
...
import { X, User, Briefcase, GraduationCap, FileText, Landmark, ScrollText } from "lucide-react";
...
export const emptyProfile: EmployeeProfile = {
...
export interface UseEmployeeProfileOptions {
  /** Skip all work until the surface is actually showing. */
  enabled?: boolean;
...
export function useEmployeeProfile({
  enabled = true, userId, employeeProfileId, userName, readOnly = false, onSuccess,
}: UseEmployeeProfileOptions) {
  ...
  }, [enabled, userId, employeeProfileId, userName, toast]);

  // Bank details are a separate table behind hr:bank_edit — fetch only when the
  // caller can actually see them, so a principal's view makes no doomed query.
  useEffect(() => {
    if (!enabled || !canEditBank || !profile.id || bankLoaded) return;
    (async () => {
  ...
  }, [enabled, canEditBank, profile.id, bankLoaded]);
  ...
  return {
    profile, setProfile, set, setAddr,
    bank, setBank,
    org, linkedUserId, loading, saving, isNew,
    uploadingPhoto, photoStage, handlePhoto, handleSave,
    canEditBank, editable,
  };
}

export type EmployeeProfileCtx = ReturnType<typeof useEmployeeProfile>;
...
/**
 * The editable record. `only` narrows the tab set — the page splits these across
 * its own Keka-style tab bar, the dialog shows them all together.
 */
export function EmployeeProfileTabs({ ctx, defaultTab = "personal" }: { ctx: EmployeeProfileCtx; defaultTab?: string }) {
  const { profile, set, setAddr, setProfile, bank, setBank, org, canEditBank, editable } = ctx;
