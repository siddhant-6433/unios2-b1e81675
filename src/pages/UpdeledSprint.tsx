...
import {
  ...
  ListPlus, BookmarkPlus, type LucideIcon,
} from "lucide-react";
import {
...
// update here too.
const DISPOSITIONS: { value: string; label: string; tone: string; icon: LucideIcon; primary?: boolean }[] = [
...
interface UpdeledSprintRpc {
  rpc(
    functionName: "updeled_sprint_queue",
    args: { p_counsellor_id: string | null; p_limit: number },
  ): Promise<{ data: QueueRow[] | null; error: RpcError | null }>;
  rpc(
    functionName: "updeled_sprint_stats",
    args: { p_counsellor_id: string | null },
  ): Promise<{ data: Stats | Stats[] | null; error: RpcError | null }>;
  rpc(
    functionName: "record_cloud_call_log",
    args: Record<string, unknown>,
  ): Promise<{ data: unknown; error: RpcError | null }>;
  rpc(
    functionName: "updeled_search_pool",
    args: { p_query: string | null; p_limit: number },
  ): Promise<{ data: PickerRow[] | null; error: RpcError | null }>;
}

interface LeadListClient {
  from(table: "lead_lists"): {
    insert(row: {
      name: string;
      source: string;
      ...
      select(columns: "id"): {
        single(): Promise<{ data: { id: string } | null; error: RpcError | null }>;
      };
    };
  };
  from(table: "lead_list_members"): {
    insert(rows: { list_id: string; lead_id: string }[]): Promise<{ error: RpcError | null }>;
  };
}
...
interface AiCallRecordsClient {
  from(table: "ai_call_records"): {
    select(columns: "status, duration_seconds, disposition, student_connected_at"): {
      eq(column: "call_uuid", value: string): {
        maybeSingle(): Promise<{ data: AiCallRecord | null; error: RpcError | null }>;
      };
    };
  };
}

const updeledSprintRpc = supabase as unknown as UpdeledSprintRpc;
const leadListClient = supabase as unknown as LeadListClient;
const aiCallRecordsClient = supabase as unknown as AiCallRecordsClient;

function errorMessage(error: unknown, fallback = "Try again"): string {
...
const CALLED_BADGE: Record<string, { label: string; tone: string; icon: LucideIcon }> = {
...
const BUCKET_META: Record<Bucket, { label: string; tone: string; icon: LucideIcon }> = {
  ...
  unreplied_whatsapp:   { label: "WA unreplied",      tone: "bg-primary/10 text-primary border-primary/25",          icon: MessageSquareWarning },
...
const UpdeledSprint = () => {
  ...
  const [registerFor, setRegisterFor] = useState<UpdeledRegisterTarget | null>(null);
  ...
  const activeCallStartTime = activeCall?.startTime;
  ...
  useEffect(() => {
    if (activeCallStatus !== "calling" && activeCallStatus !== "connected") return;
    const t = window.setInterval(() => {
      if (activeCallStartTime) setCallElapsed(Math.floor((Date.now() - activeCallStartTime) / 1000));
    }, 1000);
    return () => window.clearInterval(t);
  }, [activeCallStatus, activeCallStartTime]);
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [queueRes, statsRes] = await Promise.all([
      updeledSprintRpc.rpc("updeled_sprint_queue", {
        p_counsellor_id: counsellorParam,
        p_limit: 1000,
      }),
      updeledSprintRpc.rpc("updeled_sprint_stats", { p_counsellor_id: profileId }),
    ]);
    if (queueRes.error) {
  ...
  const persistDisposition = useCallback(async (lead: QueueRow, disposition: string, durationSeconds: number, callUuid: string | null) => {
    const finalUuid = callUuid || (typeof crypto !== "undefined" && typeof crypto.randomUUID === "function" ? crypto.randomUUID() : `manual-${Date.now()}`);
    const durStr = durationSeconds > 0 ? ` (${Math.floor(durationSeconds / 60)}m${durationSeconds % 60 ? ` ${durationSeconds % 60}s` : ""})` : "";

    await updeledSprintRpc.rpc("record_cloud_call_log", {
      p_call_uuid:     finalUuid,
      p_lead_id:       lead.lead_id,
      p_user_id:       user?.id || null,
      p_disposition:   disposition,
      p_duration:      durationSeconds,
      p_notes:         `UPDELED Sprint: ${disposition.replace("_", " ")}`,
      p_source:        "manual",
    ...
    await supabase.from("lead_activities").insert({
      ...
      description: `Call: ${disposition.replace("_", " ")}${durStr} (via UPDELED Sprint)`,
    });
  ...
  const markDisposition = useCallback(async (disposition: string) => {
    ...
    try {
      ...
      toast({ title: "Couldn't save disposition", description: errorMessage(e), variant: "destructive" });
  ...
  const startPolling = useCallback((callUuid: string, lead: QueueRow) => {
    ...
    const poll = async () => {
      const { data } = await aiCallRecordsClient
  ...
  const placeCall = useCallback(async (row: QueueRow) => {
    ...
    try {
      ...
      toast({ title: "Call failed", description: errorMessage(e), variant: "destructive" });
      setActiveCall(null);
    }
  }, [user?.id, activeCall, toast, startPolling]);

  // Save the currently filtered UPDELED queue as a reusable lead_list so the
  ...
  const handleSaveList = useCallback(async () => {
    ...
    const { data: list, error: listErr } = await leadListClient
      .from("lead_lists")
      .insert({
        name,
        source: "filter",
        filters_snapshot: filtersSnapshot,
        description: `UPDELED Sprint — ${BUCKET_FILTERS.find(b => b.key === bucketFilter)?.label || "filtered"} (${ids.length} leads)`,
      })
      .select("id")
      .single();
    ...
    for (let i = 0; i < members.length; i += 500) {
      ...
      const { error: memErr } = await leadListClient.from("lead_list_members").insert(chunk);
  ...
  const jumpToLead = useCallback((leadId: string) => {
    ...
    if (!rows.some(r => r.lead_id === leadId)) {
      // Lead is in the D.El.Ed pool but not in our current queue rows
  ...
  const cancelCall = useCallback(async () => {
    ...
    if (activeCall.callUuid) {
      try {
        ...
        toast({ title: "Cancel failed", description: errorMessage(e), variant: "destructive" });
      }
    }
    setActiveCall(null);
    setCallElapsed(0);
  ...
  return (
    <div className="space-y-4">
      {/* Sprint summary card */}
      <Card className="border-primary/25 bg-gradient-to-r from-indigo-50 to-amber-50">
        <CardContent className="p-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 text-primary font-semibold">
                <AlertTriangle className="h-5 w-5" />
                UPDELED Counselling Registration Sprint
              </div>
              <div className="text-sm text-primary/80 mt-0.5">
            ...
            <div className="flex gap-6 items-center">
              {isCounsellor && stats && (
                <div className="text-right">
                  ...
                  <div className="h-1.5 w-32 bg-primary/10 rounded mt-1 overflow-hidden">
                    <div className="h-full bg-primary/50" style={{ width: `${ownProgress}%` }} />
                  </div>
              ...
              {stats && (
                <div className="text-right">
                  ...
                  <div className="text-xs text-muted-foreground mt-0.5">of {stats.pool_total} D.El.Ed</div>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        {/* Scope toggle is shown to counsellors too: the D.El.Ed pool is a
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <Button
            ...
            title="Find any D.El.Ed lead (even outside this filter)"
          >
          ...
          {loading ? (
            ...
            scope === "mine" && (stats?.pool_remaining ?? 0) > 0 ? (
              <div className="p-10 text-center text-muted-foreground">
                ...
                <div className="font-medium">No D.El.Ed leads assigned to you.</div>
                ...
                <div className="mt-3 flex items-center justify-center gap-2">
                  <Button size="sm" onClick={() => setScope("all")} className="bg-primary hover:bg-primary/60 text-white">
                    Show whole pool
                  </Button>
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-3 px-3 py-2.5 cursor-pointer hover:bg-muted/40 transition-colors ${
                      flashLeadId === r.lead_id ? "bg-primary/15/70 border-l-2 border-primary/40"
                      : calledDisp ? "bg-slate-50/70 opacity-60 hover:opacity-100 border-l-2 border-slate-300"
                      : isSelected ? "bg-primary/5/60 border-l-2 border-primary/35"
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <Button
                        size="sm"
                        className="h-8 bg-primary hover:bg-primary/60 text-white"
      ...
      <Dialog open={showSaveList} onOpenChange={(o) => { if (!savingList) setShowSaveList(o); }}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Creates a reusable list of <strong>{filtered.length}</strong> lead{filtered.length === 1 ? "" : "s"} from the current UPDELED Sprint view. Use it from the Lists page to send bulk WhatsApp or email.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <Input
              autoFocus
              placeholder={`UPDELED — ${BUCKET_FILTERS.find(b => b.key === bucketFilter)?.label || "filtered"} — ${new Date().toLocaleDateString("en-IN", { day: "numeric", month: "short" })}`}
              value={newListName}
            ...
            <div className="ml-auto flex items-center gap-1.5 flex-wrap">
              ...
              {status === "ended" && (
                <Button
                  ...
                  className="h-8 bg-primary hover:bg-primary/60 text-white"
                  onClick={onRegister}
                  title="Skip disposition and go straight to UPDELED registration"
                >
...
// ── Lead picker dialog ────────────────────────────────────────────────────
// Searchable picker over the full D.El.Ed pool (429 leads as of writing).
...
function UpdeledLeadPicker({
  ...
  useEffect(() => {
    ...
    const handle = window.setTimeout(async () => {
      setLoading(true);
      const { data, error } = await updeledSprintRpc.rpc("updeled_search_pool", {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle>Add D.El.Ed lead to your sprint</DialogTitle>
          <DialogDescription>
            Search any lead in the UPDELED pool by name or phone. Selecting one jumps the queue to it; already-registered leads can still be viewed but can't be re-registered.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-2">
          ...
          <div className="max-h-80 overflow-y-auto rounded border divide-y">
            {loading ? (
              ...
              <div className="p-6 text-center text-sm text-muted-foreground">
                {query ? "No D.El.Ed leads match that search." : "Type to search the UPDELED pool."}
              </div>
            ) : (
              results.map(r => (
                <div key={r.lead_id} className="flex items-center gap-3 px-3 py-2 hover:bg-muted/40">
                  ...
                  {!r.registered && (
                    <Button
                      size="sm"
                      className="h-7 text-xs bg-primary hover:bg-primary/60 text-white"
