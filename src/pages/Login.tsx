...
const CACHED_GOOGLE_KEY = "unios:last_google_identity";
...
const readCachedGoogleIdentity = (): CachedGoogleIdentity | null => {
  try {
    const raw = localStorage.getItem(CACHED_GOOGLE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as Partial<CachedGoogleIdentity> | null;
...
const writeCachedGoogleIdentity = (identity: CachedGoogleIdentity) => {
  try {
    localStorage.setItem(CACHED_GOOGLE_KEY, JSON.stringify(identity));
  } catch {
...
const Login = () => {
  ...
  useEffect(() => {
    ...
    const identities = (session.user as { identities?: Array<{ provider: string; identity_data?: Record<string, unknown> }> }).identities;
    const google = identities?.find((i) => i.provider === "google");
    const meta = (google?.identity_data || session.user.user_metadata || {}) as Record<string, unknown>;
    const email = (typeof meta.email === "string" ? meta.email : undefined) || session.user.email;
    if (!email || !google) return;
    const cache: CachedGoogleIdentity = {
    ...
    writeCachedGoogleIdentity(cache);
  }, [session]);
  ...
  const [cachedGoogle, setCachedGoogle] = useState<CachedGoogleIdentity | null>(() => readCachedGoogleIdentity());
  ...
  const handleGoogleLogin = async (loginHint?: string) => {
    setSubmitting(true);
    try {
      const queryParams: Record<string, string> = loginHint
  ...
  const handleUseDifferentGoogleAccount = () => {
    try {
      localStorage.removeItem(CACHED_GOOGLE_KEY);
    } catch {
      /* ignore */
    }
    setCachedGoogle(null);
  };
            ...
            <div className="space-y-4">
              ...
              {waSignInState !== "no_account" && (
                <div className="text-center">
                  <button
                    ...
                    className="text-xs text-muted-foreground hover:text-emerald-700 hover:underline transition-colors"
                  >
              ...
              {cachedGoogle && (
                <div className="space-y-1.5">
                  <button
                    type="button"
                    onClick={() => handleGoogleLogin(cachedGoogle.email)}
                    disabled={submitting}
                    className="w-full rounded-xl border border-input bg-card px-4 py-3 text-left hover:bg-muted transition-colors disabled:opacity-50 flex items-center gap-3"
                  >
                    {cachedGoogle.picture ? (
                      <img
                        src={cachedGoogle.picture}
                        alt=""
                        referrerPolicy="no-referrer"
                        className="h-9 w-9 rounded-full shrink-0 bg-muted object-cover"
                      />
                    ) : (
                      <div className="h-9 w-9 rounded-full shrink-0 bg-primary/10 text-primary flex items-center justify-center text-sm font-semibold">
                        {(cachedGoogle.name || cachedGoogle.email)[0]?.toUpperCase()}
                      </div>
                    )}
                    <span className="flex min-w-0 flex-1 flex-col">
                      <span className="truncate text-sm font-medium text-foreground">
                        Sign in as {cachedGoogle.name?.split(" ")[0] || cachedGoogle.email}
                      </span>
                      <span className="truncate text-xs text-muted-foreground">{cachedGoogle.email}</span>
                    </span>
                    {submitting ? (
              ...
              <div className="text-center">
                <button
                  type="button"
                  onClick={() => { resetState(); setMethod("email_otp"); }}
