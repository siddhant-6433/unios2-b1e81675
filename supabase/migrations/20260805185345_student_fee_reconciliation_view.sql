...
-- version=20260805185345 name=student_fee_reconciliation_view applied_by=siddhant@nimt.ac.in
-- Git must keep this timestamp so `db push` matches remote history.

-- Replaces the first cut, which compared each payment against its
-- fee_ledger_payments links. That over-reported badly: the edge-function
-- provisioning path credits fee_ledger.paid_amount without always writing a
-- link row, so ~₹5.8L of money that IS on the ledger looked missing.
--
-- This compares the two totals that actually matter per student:
--   confirmed payments on the lead  vs  what the ledger records as paid.
DROP VIEW IF EXISTS public.unapplied_lead_payments;
...
SELECT s.id                                   AS student_id,
       ...
       CASE
         WHEN s.lead_id IS NULL AND orphan.lead_id IS NOT NULL THEN 'orphaned_student_lead_not_linked'
  ...
  -- A student with no lead_id whose phone matches a lead carrying money: the
  -- case where a manually added student is financially disconnected.
  LEFT JOIN LATERAL (
        SELECT l.id AS lead_id,
               (SELECT COALESCE(SUM(lp2.amount),0) FROM public.lead_payments lp2
