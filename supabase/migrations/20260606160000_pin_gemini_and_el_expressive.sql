-- Two related fixes bundled (both come from the same /loop testing
-- session — apply together):
--
-- 1. Pin gemini_model away from the unstable `-latest` alias. Google
--    rotates `-latest` without notice; calls were dropping mid-stream
--    with WS code 1008 "Operation is not implemented" once the alias
--    flipped to a model variant that doesn't accept our setup payload.
--    preview-09-2025 has been live the longest of the available dated
--    snapshots and is least likely to be deprecated mid-quarter.
--
-- 2. Make ElevenLabs voice_settings (style / stability / similarity_boost)
--    + model_id admin-tunable. Why:
--       - Filler-ack clips ("ji, ek second") were rendered with EL defaults
--         and sounded louder/odder than the main response. With these as
--         tunable settings AND included in the filler-cache signature, the
--         filler always re-renders with whatever the main response uses.
--       - "Expressive mode" maps to a higher style value (~0.4) and the
--         eleven_v3 model. Both make Anjura sound noticeably more human
--         on Hinglish, at the cost of ~30% more TTS latency. Default left
--         conservative — admin can dial up.
...
SET value = (
  value::jsonb || jsonb_build_object(
    'gemini_model', CASE
      ...
      ELSE 'gemini-2.5-flash-native-audio-preview-09-2025'
    END,
    'elevenlabs_style',       COALESCE((value::jsonb->>'elevenlabs_style')::numeric, 0.4),
    'elevenlabs_stability',   COALESCE((value::jsonb->>'elevenlabs_stability')::numeric, 0.45),
    'elevenlabs_similarity',  COALESCE((value::jsonb->>'elevenlabs_similarity')::numeric, 0.75),
    'elevenlabs_model',       COALESCE(value::jsonb->>'elevenlabs_model', 'eleven_turbo_v2_5')
  )
)::text
WHERE key = 'voice_agent_settings';

-- ============================================================
-- 2. Extend the RPC validation allowlist
-- ============================================================
