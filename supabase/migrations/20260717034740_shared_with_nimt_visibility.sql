...
-- version=20260717034740 name=shared_with_nimt_visibility applied_by=siddhant@nimt.ac.in
...
AS $function$
  WITH classified AS (
    SELECT
      ...
      l.stage::text AS stage, l.source::text AS source,
      l.course_id, l.campus_id, l.created_at,
      l.lead_score, l.lead_temperature,
      c.name AS course_name, cam.name AS campus_name,
      ...
      EXISTS (
        ...
        WHERE a.lead_id = l.id
          AND (a.payment_status = 'paid' OR a.submitted_at IS NOT NULL
    ...
    LEFT JOIN public.institutions cam_inst
      ON cam_inst.campus_id = l.campus_id AND cam_inst.type = 'school'
  ...
  latest_ai AS (
    SELECT DISTINCT ON (lead_id) lead_id, summary, disposition, conversion_probability
    FROM public.ai_call_records WHERE summary IS NOT NULL
    ORDER BY lead_id, created_at DESC
  ),
  latest_queue AS (
    SELECT DISTINCT ON (lead_id) lead_id, status, error_message, scheduled_at, created_at
    FROM public.ai_call_queue ORDER BY lead_id, created_at DESC
  )
  SELECT
    ...
    CASE
      ...
      ELSE 'nimt'
    END AS school_brand,
    cl.jd_category, cl.ai_called, cl.skip_ai_call,
