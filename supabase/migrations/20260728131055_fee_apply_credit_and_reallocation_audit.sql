...
-- version=20260728131055 name=fee_apply_credit_and_reallocation_audit applied_by=siddhant@nimt.ac.in
...
CREATE TABLE IF NOT EXISTS public.fee_ledger_reallocation_audit (
  ...
  from_fee_ledger_id uuid, to_fee_ledger_id uuid,
  from_fee_code text, from_term text, to_fee_code text, to_term text,
  amount numeric(12,2) NOT NULL CHECK (amount > 0),
  reason text, actor_user_id uuid, actor_role text,
...
CREATE OR REPLACE FUNCTION public.apply_student_credit(
  _id uuid, _fee_ledger_id uuid DEFAULT NULL, _amount numeric DEFAULT NULL, _reason text DEFAULT NULL
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $function$
DECLARE
  v_lead uuid; v_student uuid; v_form uuid; v_credit numeric; v_budget numeric; v_applied numeric := 0;
  v_row RECORD; v_apply numeric; v_before numeric; v_actor uuid := auth.uid(); v_role text;
BEGIN
  ...
  FOR v_row IN
    SELECT fl.id, fl.total_amount, fl.concession, fl.paid_amount, fl.due_date, fl.status, fc.code, fl.term
     ...
     ORDER BY CASE WHEN UPPER(fc.code) LIKE '%SEC' THEN 1 WHEN fl.term = 'admission' THEN 0 ELSE 2 END, fl.due_date, fl.id
  LOOP
    ...
    INSERT INTO public.fee_ledger_reallocation_audit
      (student_id, action, to_fee_ledger_id, to_fee_code, to_term, amount, reason, actor_user_id, actor_role, before_json, after_json)
