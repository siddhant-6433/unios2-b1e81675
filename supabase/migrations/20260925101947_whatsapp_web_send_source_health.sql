-- Keep WhatsApp Web handoffs distinct from API delivery analytics.
ALTER TABLE public.whatsapp_messages
  ADD COLUMN IF NOT EXISTS send_source text NOT NULL DEFAULT 'meta_api'
    CHECK (send_source IN ('meta_api', 'whatsapp_web'));

COMMENT ON COLUMN public.whatsapp_messages.send_source IS
  'Origin of the outbound action. whatsapp_web is a counsellor-opened, prefilled Web chat and is not delivery-confirmed by Meta.';

CREATE INDEX IF NOT EXISTS idx_whatsapp_messages_send_source_created
  ON public.whatsapp_messages (send_source, created_at DESC)
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'web_sends',       (SELECT count(*) FROM public.whatsapp_messages WHERE direction = 'outbound' AND send_source = 'whatsapp_web' AND created_at >= v_since AND created_at < v_until),
    'since',           v_since,
    'until',           v_until,
    'total',           count(*),
  ...
  WHERE direction = 'outbound' AND COALESCE(send_source, 'meta_api') = 'meta_api' AND created_at >= v_since AND created_at < v_until;
  ...
  FROM (
    SELECT
      ...
      count(*) AS failures
    FROM public.whatsapp_messages
    WHERE direction = 'outbound' AND COALESCE(send_source, 'meta_api') = 'meta_api'
      AND status = 'failed'
  ...
  FROM (
    SELECT
      ...
      lead_id
    FROM public.whatsapp_messages
    WHERE direction = 'outbound' AND COALESCE(send_source, 'meta_api') = 'meta_api' AND status = 'failed'
