...
-- version=20260823080126 name=student_guardian_email_login applied_by=siddhant@nimt.ac.in
...
DECLARE
  v_id uuid;
BEGIN
  FOR v_id IN SELECT id FROM public.students WHERE admission_no IS NOT NULL LOOP
    BEGIN
      PERFORM public.backfill_student_from_application(v_id);
    EXCEPTION WHEN OTHERS THEN
      RAISE WARNING '[backfill guardian_email] student % skipped: %', v_id, SQLERRM;
    END;
  END LOOP;
END$$;
