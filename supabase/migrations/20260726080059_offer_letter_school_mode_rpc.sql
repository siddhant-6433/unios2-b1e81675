...
-- version=20260726080059 name=offer_letter_school_mode_rpc applied_by=siddhant@nimt.ac.in
...
BEGIN
  ...
  INSERT INTO public.offer_letters (
    lead_id, total_fee, scholarship_amount, net_fee, token_fee_amount,
    token_fee_user_edited, admission_mode, entrance_exam_name, acceptance_deadline,
    course_id, campus_id, session_id, issued_by, approval_status, approved_by, approved_at,
    student_type, hostel_type, transport_zone
  )
  VALUES (
    v_lead.id, _total_fee, 0, _net_fee, _token_fee_amount,
    COALESCE(_token_fee_user_edited, false), COALESCE(_admission_mode, 'direct'),
    CASE WHEN COALESCE(_admission_mode, 'direct') = 'entrance' THEN NULLIF(trim(_entrance_exam_name), '') ELSE NULL END,
    _acceptance_deadline, v_lead.course_id, v_lead.campus_id, _session_id, v_uid, 'approved', v_uid, now(),
  ...
  IF v_lead.stage::text NOT IN ('token_paid', 'pre_admitted', 'admitted', 'rejected', 'ineligible', 'dnc') THEN
    UPDATE public.leads
      SET stage = 'offer_sent'::public.lead_stage, offer_amount = _net_fee, updated_at = now()
    WHERE id = v_lead.id;
  END IF;

  INSERT INTO public.lead_activities (lead_id, user_id, type, description, old_stage, new_stage)
  VALUES (
    v_lead.id, v_profile_id, 'offer',
    ...
    CASE WHEN v_old_stage::text NOT IN ('token_paid', 'pre_admitted', 'admitted', 'rejected', 'ineligible', 'dnc')
      THEN 'offer_sent'::public.lead_stage ELSE v_old_stage END
  );

  INSERT INTO public.application_on_behalf_audit (
    action, lead_id, application_uuid, application_id, offer_letter_id,
    actor_user_id, academic_partner_id, candidate_phone, metadata
  )
  VALUES (
    'offer_issued_by_partner', v_lead.id, v_app.id, v_app.application_id, v_offer_id,
    v_uid, v_partner_id, v_lead.phone,
    jsonb_build_object('total_fee', _total_fee, 'net_fee', _net_fee,
      'token_fee_amount', _token_fee_amount, 'admission_mode', COALESCE(_admission_mode, 'direct'))
  );

  RETURN jsonb_build_object(
    'offer_letter_id', v_offer_id, 'lead_id', v_lead.id,
    'application_id', v_app.application_id, 'approval_status', 'approved'
  );
END;
$function$;
...
BEGIN
  ...
  VALUES (
    ...
    COALESCE(_token_fee_user_edited, false), COALESCE(_admission_mode, 'direct'),
    CASE WHEN COALESCE(_admission_mode, 'direct') = 'entrance' THEN NULLIF(trim(_entrance_exam_name), '') ELSE NULL END,
    _acceptance_deadline, v_offer_course_id, v_offer_campus_id, _session_id, v_uid, 'approved', v_uid, now(),
    NULLIF(trim(COALESCE(_student_type, '')), ''),
    NULLIF(trim(COALESCE(_hostel_type, '')), ''),
    NULLIF(trim(COALESCE(_transport_zone, '')), '')
  )
