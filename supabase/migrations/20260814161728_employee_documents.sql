...
-- version=20260814161728 name=employee_documents applied_by=siddhant@nimt.ac.in
...
CREATE INDEX IF NOT EXISTS employee_documents_expiring_idx
  ON public.employee_documents (expires_on) WHERE expires_on IS NOT NULL;
...
CREATE POLICY "Employees upload their own documents"
  ...
  WITH CHECK (
    status = 'pending' AND verified_by IS NULL
...
CREATE OR REPLACE FUNCTION public.review_employee_document(
  _document_id uuid, _status text, _note text DEFAULT NULL
) RETURNS void
...
RETURNS TABLE (code text, name text, folder text, is_mandatory boolean, status text, review_note text)
