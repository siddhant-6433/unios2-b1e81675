...
-- version=20260810102526 name=repoint_guc_triggers_to_app_config applied_by=siddhant@nimt.ac.in
...
AS $$
DECLARE
  v_token uuid; v_lead record; v_course_name text; v_phone text;
  v_deadline_str text; v_net_fee_str text; v_supabase_url text; v_service_key text;
BEGIN
  ...
  IF v_supabase_url IS NULL OR v_service_key IS NULL THEN
    RAISE NOTICE 'Skipping offer WhatsApp send — _app_config url/key missing'; RETURN NEW;
  END IF;
...
BEGIN
  ...
  IF v_url IS NULL OR v_key IS NULL THEN
    RAISE NOTICE 'Skipping automation-engine call — _app_config url/key missing'; RETURN NEW;
  END IF;
  ...
  IF OLD.stage IS DISTINCT FROM NEW.stage THEN
    PERFORM net.http_post(
      ...
      body := jsonb_build_object('trigger_type','stage_change','lead_id',NEW.id,'old_stage',OLD.stage::text,'new_stage',NEW.stage::text));
  END IF;
  ...
  RETURN NEW;
EXCEPTION WHEN OTHERS THEN
  RAISE WARNING 'Automation trigger failed: %', SQLERRM; RETURN NEW;
END;
$$;
