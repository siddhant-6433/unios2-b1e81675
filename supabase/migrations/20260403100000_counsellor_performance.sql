-- Sprint 2 Feature 1: Counsellor Performance Dashboard

-- Targets table
CREATE TABLE public.counsellor_targets (
  ...
  whatsapps_target integer DEFAULT 100,
  followups_target integer DEFAULT 30,
  visits_target integer DEFAULT 10,
  conversions_target integer DEFAULT 5,
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now(),
  UNIQUE(counsellor_id, period, period_start)
);

ALTER TABLE public.counsellor_targets ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Authenticated users can manage counsellor_targets"
  ON public.counsellor_targets FOR ALL TO authenticated USING (true) WITH CHECK (true);

-- Performance stats view (parameterized queries will filter by date range at query time)
-- Roles live in user_roles table, not profiles
CREATE OR REPLACE VIEW public.counsellor_performance_stats AS
SELECT
...
GRANT SELECT ON public.counsellor_performance_stats TO authenticated;
