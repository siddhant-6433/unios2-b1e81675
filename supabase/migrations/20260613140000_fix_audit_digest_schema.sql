...
-- audit trigger function pinned search_path=public, so unqualified
-- digest() resolved to nothing → "function digest(text, unknown) does
-- not exist", which rolled back any lead_payments INSERT/UPDATE/DELETE.
-- Symptom: offline Token Fee recording failed at the audit trigger
-- after the pg_net body cast was fixed.
--
-- Fix: extend search_path to include `extensions`, and qualify the call
-- with extensions.digest(... ::bytea, ... ::text) so the overload is
