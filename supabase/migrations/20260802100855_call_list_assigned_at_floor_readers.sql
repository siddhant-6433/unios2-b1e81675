-- Floor the assigner rollup and the per-lead Calling Report on the member's
-- assigned_at / the assignment row's created_at, so a call made before the
-- hand-off can never be reported as this cycle's outcome.
...
BEGIN
  ...
  -- Floored on the member's own assigned_at: a call made before this list was
  -- handed out — including one made for a different list earlier — is a
  -- previous cycle's call and contributes nothing here.
  latest_call AS (
  ...
  totals AS (
    SELECT list_id,
           ...
           -- Real hand-off time, not the date the list was first created.
...
-- ── 4. Per-lead Calling Report: same floor ───────────────────────────────────
-- The LATERAL "latest call" showed the newest call/AI-call for the lead
-- regardless of when it happened, so a months-old disposition could display as
-- the result of a list assigned this morning. lead_assignment_history.created_at
-- is that row's own hand-off instant, so it is the correct floor here.
-- Otherwise identical to 20260701100000.
