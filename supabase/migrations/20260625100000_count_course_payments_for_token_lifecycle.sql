-- Course-fee payments must satisfy the token lifecycle gate.
--
-- Applicant one-time payment buttons record first-year/full-course payments as
-- lead_payments.type = 'other'. Those rows are already applied to the fee
-- ledger, but lead_fee_status previously counted only type='token_fee' for
-- token_complete. Result: a candidate could pay enough course money to cross
-- PAN/AN thresholds and still remain stuck at offer_sent.
...
BEGIN
  SELECT CASE WHEN c.code IN ('DAOTT-GN','OTT-GN') THEN 4000 ELSE NULL END
    INTO v_min_instalment
    FROM public.leads l
    LEFT JOIN public.courses c ON c.id = l.course_id
   WHERE l.id = _lead_id;

  v_min_instalment := COALESCE(v_min_instalment, (v_policy->>'min_token_instalment')::numeric, 5000);
