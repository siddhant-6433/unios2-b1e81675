...
-- "interview" alone = job, "admission interview" = admission
-- "training" alone = job, "clinical training" = admission
-- "offer" alone = job, "offer letter" = admission
...
BEGIN
  ...
  -- ═══ JOB SIGNALS ═══
  -- Strong job keywords (always job unless overridden by admission context)
  IF v_msg ~* '(vacancy|hiring|naukri|rozgar|kaam\s+chahiye|kaam\s+mil|recruit|staff\s+required|teacher\s+wanted|faculty\s+required|peon|driver|guard|helper|clerk|office\s+boy|receptionist|resume|cv\s|ctc|stipend|in[\s-]?hand|work\s+from\s+home)' THEN
    v_job_score := v_job_score + 3;
  END IF;
  -- Moderate job keywords (job only if NOT near admission context)
  IF v_msg ~* '\m(job|salary|working|joining|fresher|employ|position|openings?|intern)\M' THEN
    v_job_score := v_job_score + 2;
  END IF;
  -- Context-sensitive: "interview" = job UNLESS preceded by "admission"
  IF v_msg ~* '\minterview\M' AND NOT v_msg ~* '(admission\s+interview|campus\s+interview|interview\s+round|interview\s+for\s+admission)' THEN
    v_job_score := v_job_score + 2;
  END IF;
  -- Context-sensitive: "training" = job UNLESS clinical/nursing context
  IF v_msg ~* '\mtraining\M' AND NOT v_msg ~* '(clinical\s+training|nursing\s+training|hospital\s+training|internship\s+training|practical\s+training)' THEN
    v_job_score := v_job_score + 1;
  END IF;
  -- Context-sensitive: "offer" = job UNLESS "offer letter" / admission context
  IF v_msg ~* '\moffer\M' AND NOT v_msg ~* '(offer\s+letter|admission\s+offer|seat\s+offer)' THEN
    v_job_score := v_job_score + 1;
  END IF;
  -- Context-sensitive: "experience" = job UNLESS "years experience" in education context
  IF v_msg ~* '\mexperience\M' AND NOT v_msg ~* '(year.*experience|learning\s+experience|campus\s+experience|student\s+experience)' THEN
    v_job_score := v_job_score + 1;
  END IF;
  ...
  -- Admission context overrides job signals (someone asking about placement packages is still admission)
  ...
  -- Step 2: Learned keywords (if scoring didn't decide)
  ...
  -- Log training for learning model
...
-- Re-run backfill with context-aware scoring
DO $$
DECLARE
  r RECORD;
  v_result text;
...
BEGIN
  ...
  LOOP
    ...
    END IF;
  END LOOP;
  RAISE NOTICE 'Context-aware backfill: % job, % vendor', v_job, v_vendor;
END $$;
