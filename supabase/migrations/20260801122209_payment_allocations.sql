-- ====================================================================
-- Per-head payment allocations ("breakup").
--
-- Staff can build a breakup of variable amounts across specific fee heads
-- (fee_codes) on a single total payment link or offline receipt. The breakup
-- rides on the payment and is mapped to the student's real fee_ledger rows
-- when they exist (at admission / when the head is provisioned). This lets a
-- payment land on Transport / Meal / any ad-hoc head instead of the default
-- type-string sweep (application_fee -> app-fee row; else -> tuition earliest-due).
--
-- Shape of `allocations`: [{ "fee_code_id": uuid, "amount": numeric, "label": text }]
-- Sum should equal the payment amount (validated in the app); any leftover is
-- left as unallocated general credit.
-- ====================================================================
...
COMMENT ON COLUMN public.lead_payments.allocations IS
  'Optional per-head breakup [{fee_code_id, amount, label}]. When set, provision_student_fees credits the matching fee_ledger heads (earliest-due, with spillover) instead of the default type-string routing.';

-- Re-create provision_student_fees with an allocation-aware branch. Behaviour is
-- unchanged for payments with allocations = NULL (all existing rows). Additive.
  ...
  LOOP
    ...
    IF v_lp.allocations IS NOT NULL AND jsonb_array_length(v_lp.allocations) > 0 THEN
      ...
      FOR v_alloc IN SELECT * FROM jsonb_array_elements(v_lp.allocations) LOOP
        ...
        IF v_fc IS NULL OR v_alloc_amt <= 0 THEN CONTINUE; END IF;
