-- DAOTT/DOTT Stetho Batch admission collection update.
--
-- PAN requires Rs 5,000 total: application fee Rs 1,000 + seat-block payment
-- Rs 4,000. AN requires another Rs 10,000 within the 5-day hold, so the
-- course-paid threshold for AN is Rs 15,000. Semester 1 remains Rs 40,000 and
-- later semesters remain unchanged.
...
DECLARE
  v_seat_code_id uuid;
  v_tuition_code_id uuid;
BEGIN
  SELECT id INTO v_seat_code_id
    FROM public.fee_codes
   WHERE code = 'DAOTT-SEAT';

  SELECT id INTO v_tuition_code_id
    FROM public.fee_codes
   WHERE code = 'DAOTT-TUITION';

  IF v_seat_code_id IS NULL OR v_tuition_code_id IS NULL THEN
    RAISE NOTICE 'DAOTT fee codes not found; skipping DAOTT application + seat-block correction';
    RETURN;
  END IF;
  ...
  UPDATE public.fee_structure_items fsi
    ...
    JOIN public.courses c ON c.id = fs.course_id
   WHERE fsi.fee_structure_id = fs.id
     AND fsi.fee_code_id = v_tuition_code_id
     AND fsi.term = 'year_1'
     AND fs.version = 'stetho_batch'
     AND c.code IN ('DAOTT-GN','OTT-GN');
  ...
  UPDATE public.fee_structures fs
     SET metadata = jsonb_set(
       ...
       '{year_1,payment_note}',
       to_jsonb('Application fee Rs 1,000 + seat block Rs 4,000 unlocks PAN; additional Rs 10,000 within 5 days unlocks AN; Sem 1 total Rs 40,000'::text),
       true
     ),
         policy = COALESCE(fs.policy, '{}'::jsonb) || jsonb_build_object(
           'token_required_amount', 5000,
           'an_threshold_amount', 15000,
           'min_token_instalment', 4000,
  ...
  UPDATE public.offer_letters ol
     SET token_fee_amount = 5000
    FROM public.leads l
    JOIN public.courses c ON c.id = l.course_id
   WHERE ol.lead_id = l.id
     AND c.code IN ('DAOTT-GN','OTT-GN')
     AND ol.approval_status = 'approved'
     AND COALESCE(ol.token_fee_amount, 0) IN (4000, 10000);
END $$;
...
ALTER TABLE public.lead_payments
  ...
  CHECK (type <> 'token_fee' OR amount > 0)
  NOT VALID;
