...
-- version=20260811044912 name=whatsapp_send_triggers_use_cron_secret applied_by=siddhant@nimt.ac.in
...
DECLARE
  ...
  v_deadline_str text; v_net_fee_str text; v_supabase_url text; v_cron_secret text;
BEGIN
...
BEGIN
  ...
  PERFORM net.http_post(
    ...
    headers := jsonb_build_object('Content-Type','application/json','x-cron-secret', v_cron_secret),
    body    := jsonb_build_object(
      'template_key', 'student_portal_invite', 'phone', v_tok.phone, 'lead_id', v_tok.lead_id,
