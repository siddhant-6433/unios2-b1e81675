-- Fee Collection vs Due report: per student, per parent fee head, due vs collected.
-- Mounted at Finance → Reports → Collection vs Due.
--
-- Scopes:
--   till_date     heads whose due_date is on or before _as_of (future terms out).
--                 Heads with no due date are treated as already billed.
--   entire_batch  every provisioned parent head, including future terms
--   overdue       unpaid parent heads whose due_date is before _as_of
--
-- LATE-FEE ledger rows are not returned as their own heads. Their due/paid
-- amounts hang off the parent head (term 'late_' || parent.term) for the
-- detailed view. Late fines are keyed student+term (not per fee head), so they
-- are attached to exactly one parent row of that term to avoid double-counting.
-- Collected date is the latest confirmed lead_payment applied to that ledger
-- row, in IST.
...
BEGIN
  ...
  IF v_scope NOT IN ('till_date', 'entire_batch', 'overdue') THEN
    v_scope := 'till_date';
  END IF;
