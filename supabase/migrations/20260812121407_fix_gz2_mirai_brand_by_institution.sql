-- Fix: GZ2/Avantika campus is shared between Mirai Experiential School and the
-- College of Education (B.Ed). get_unassigned_leads_bucket() labelled the
-- school_brand purely by campus_id (= GZ2 → 'mirai'), which mislabels every
-- B.Ed–Avantika school-bucket lead as Mirai. Derive the brand from the course's
-- OWNING INSTITUTION instead (courses → departments → institutions), the same
-- discriminator generate-offer-letter and generate-apply-link use.
--
-- Mirai institution id: d8c95a30-ecc6-4b41-8bed-987c960dc44a.
--
-- Only change vs the prior definition (20260717130000): expose
-- departments.institution_id in the classified CTE and switch the school_brand
-- CASE from campus_id to institution_id. The `bucket` derivation is unchanged —
-- it already prefers the course's institution type (i.type) and only falls back
-- to the campus-school signal when a lead has no resolvable course.
...
AS $function$
  ...
  SELECT
    ...
    CASE
      ...
      WHEN cl.institution_id = 'd8c95a30-ecc6-4b41-8bed-987c960dc44a'::uuid THEN 'mirai'
      ELSE 'nimt'
    END AS school_brand,
    cl.jd_category,
    cl.ai_called,
