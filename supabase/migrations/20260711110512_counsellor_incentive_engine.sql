...
-- version=20260711110512 name=counsellor_incentive_engine applied_by=siddhant@nimt.ac.in
...
BEGIN
  ...
  LOOP
    ...
    SELECT count(*) INTO v_adm_count FROM public.incentive_ledger
    WHERE counsellor_id = v_c.counsellor_id AND month = v_month AND component = 'base' AND amount >= 0;
    ...
    IF v_eligible AND v_mult <> 1.0 THEN
      FOR v_base_row IN
        SELECT id, lead_id, amount, calc_inputs FROM public.incentive_ledger
        WHERE counsellor_id = v_c.counsellor_id AND month = v_month AND component = 'base' AND amount >= 0
          AND (calc_inputs->>'source_class') <> 'consultant'
      LOOP
        INSERT INTO public.incentive_ledger (counsellor_id, lead_id, month, component, amount, calc_inputs)
    ...
    SELECT count(*) FILTER (WHERE (calc_inputs->>'qualifying_date')::timestamptz >= (v_month + interval '1 month' - interval '3 days')),
           count(*)
