-- Add optional _consultant_id to insert_lead so the Add Lead dialog can attach
-- a consultant when Source = 'consultant'. Drop + recreate to extend signature.

DROP FUNCTION IF EXISTS public.insert_lead(text, text, text, text, text, text, uuid, uuid, uuid, text);
...
BEGIN
  ...
  IF v_lead_id IS NOT NULL THEN
    UPDATE leads SET
      ...
      consultant_id = COALESCE(_consultant_id, consultant_id),
      updated_at = now()
    WHERE id = v_lead_id;
    RETURN v_lead_id;
  END IF;
