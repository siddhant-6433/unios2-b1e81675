...
-- version=20260825182337 name=abvmu_settle_finance_role applied_by=siddhant@nimt.ac.in
...
BEGIN
  ...
  INSERT INTO public.lead_payments (
    ...
    v_claim.lead_id, 'other', v_claim.amount, 'confirmed', 'bank_transfer',
  ...
  SET status = 'settled', settled_by = v_uid, settled_at = now(),
      settlement_payment_id = v_payment_id, settlement_notes = NULLIF(trim(_notes), ''), updated_at = now()
  WHERE id = _claim_id;

  INSERT INTO public.lead_activities (lead_id, user_id, type, description)
  VALUES (v_claim.lead_id, (SELECT id FROM public.profiles WHERE user_id = v_uid LIMIT 1), 'info_update',
    'ABVMU deposit settled — payment ' || v_receipt || ' for ₹' || to_char(v_claim.amount, 'FM9G99G99G990'));

  RETURN jsonb_build_object('id', _claim_id, 'status', 'settled', 'payment_id', v_payment_id, 'receipt_no', v_receipt);
END;
$$;
