...
-- version=20260813172548 name=cold_lead_revival_cycle_part2 applied_by=siddhant@nimt.ac.in
-- Git must keep this timestamp so `db push` matches remote history.

-- Cold lead revival cycle (part 2 of 2): cycle state view, daily cron, and the
-- cold tab in pending_followups_payload.
  ...
  UPDATE public.leads
   ...
   WHERE id IN (
     SELECT c.lead_id FROM public.cold_cycle_state c
      WHERE NOT c.has_open_round AND NOT c.responded AND c.rounds_spent >= 2
   );
