-- Stamp lead_list_members.assigned_at on every assignment. This is the
-- load-bearing half of the previous-cycle fix: without it assigned_at stays
-- NULL and every reader's floor falls back to "no floor".
-- Superseded in-place by the re-entrant version in the next migration;
-- kept because this version is recorded in schema_migrations.
