-- Materialize WhatsApp list recipients server-side.
--
-- The Lists UI may not be allowed to embedded-select every lead in a saved
-- list because lead visibility is governed by lead RLS. Campaign recipient
-- creation must use the list membership itself as the source of truth, while
-- still skipping DNC and no-phone leads.
