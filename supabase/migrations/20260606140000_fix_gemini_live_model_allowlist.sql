-- Google deprecated `gemini-2.0-flash-live-001` and the
-- `gemini-2.5-flash-preview-native-audio-dialog` model name. Calls were
-- failing with WS code 1008 "models/gemini-2.0-flash-live-001 is not
-- found for API version v1beta, or is not supported for
-- bidiGenerateContent" because the admin had `gemini-2.0-flash-live-001`
-- saved.
--
-- Currently-supported Live models per Google's models.list endpoint:
...
--
-- This migration:
--   1) Heals the live row — switches any deprecated model to the safe
--      `-latest` alias so calls resume immediately.
--   2) Updates the set_voice_agent_settings RPC allowlist to match
--      what Google actually serves today.
...
-- Drop and recreate with corrected allowlist. Function body is preserved
-- verbatim from 20260606120000 except the gemini_model IN (...) check.
