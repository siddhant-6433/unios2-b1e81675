-- Remove the heavyweight counsellor_tat_defaults view from the every-page badge hot path.
-- Dashboard pages can still load that view explicitly; sidebar badges stay scoped and cheap.
...
BEGIN
  ...
  -- Do not read counsellor_tat_defaults here. That view scans all counsellors
  -- and their lead activity, which made every sidebar render expensive. Keep the
  -- hot-path count scoped to already-resolved lead IDs; dashboard pages can load
  -- the full TAT view when the user explicitly opens the dashboard.
  IF v_scope_counsellor_id IS NOT NULL AND cardinality(v_lead_ids) > 0 THEN
