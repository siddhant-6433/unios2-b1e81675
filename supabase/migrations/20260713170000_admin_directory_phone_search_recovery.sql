-- Admin directory phone search recovery
--
-- Problem: profiles can keep profiles_phone_unique after soft-delete or archive.
-- admin_user_directory excluded deleted_at / archived rows, so searching a phone
-- on Leads & Applicants returned empty even though Edit Mobile found the owner.
--
-- Fix:
--  1) Digit-normalized phone match (last 10 digits) so 9717554419 matches +919717554419
--  2) When the search is phone-like (≥10 digits), surface matching profiles even if
--     soft-deleted or archived, and regardless of category — so admins can reclaim
--     the number. Results include deleted_at so the UI can label them.

DROP FUNCTION IF EXISTS public.admin_user_directory(boolean, text, text, text, text, int, int);
...
COMMENT ON FUNCTION public.admin_user_directory(boolean, text, text, text, text, int, int) IS
  'Paginated admin user directory. Phone searches (≥10 digits) also surface soft-deleted/archived profiles that still hold profiles_phone_unique.';
