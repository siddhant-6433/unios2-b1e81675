-- Restore DAOTT/DOTT Stetho Batch to the source fee card:
--   Sem 1: seat block 5,000 + tuition 25,000 + admin/tech 5,000 + exam 5,000
--   Sem 1 total: 40,000; programme total: 1,85,000.
--
-- This intentionally supersedes the earlier 4,000 seat-block correction.
...
BEGIN
  ...
  IF v_seat_code_id IS NULL THEN
    RAISE NOTICE 'DAOTT-SEAT fee code not found; skipping DAOTT Stetho fee-card restore';
    RETURN;
  END IF;
  ...
  UPDATE public.fee_structures fs
     SET metadata = jsonb_set(
       ...
       '{year_1,payment_note}',
       to_jsonb('Seat block Rs 5,000 + tuition Rs 25,000 + admin & technology Rs 5,000 + examination Rs 5,000'::text),
       true
     )
  ...
  UPDATE public.offer_letters ol
   ...
   WHERE ol.lead_id = l.id
     ...
     AND COALESCE(ol.token_fee_amount, 0) = 4000
...
BEGIN
  v_min_instalment := COALESCE((v_policy->>'min_token_instalment')::numeric, 5000);
