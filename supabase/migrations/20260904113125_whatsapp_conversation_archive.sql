...
-- version=20260904113125 name=whatsapp_conversation_archive applied_by=siddhant@nimt.ac.in
...
-- (phone, business_number). NOTE: whatsapp_inbox_page's own "business_number"
-- column (resolved_business_number = coalesce(business_phone_number,
-- business_phone_number_id, 'unknown')) uses a different priority order than
-- the canonical whatsapp_conversation_key() format the mode/state rows on
-- this table use elsewhere (get_whatsapp_conversations, whatsapp_reply_state_counts).
-- Archiving intentionally keys off resolved_business_number — the exact value
-- the inbox already hands back to the caller — so writes and reads stay
-- self-consistent, at the cost of potentially living on a second row from the
-- legacy mode/state row for the same conversation. That's fine: this feature
-- never touches mode/state/lead_id.
...
as $$
  with base as (
    ...
    left join public.whatsapp_conversation_state cas
     ...
     and cas.business_number = coalesce(nullif(wc.business_phone_number, ''), nullif(wc.business_phone_number_id, ''), 'unknown')
  ), scored as (
    select
      base.*,
