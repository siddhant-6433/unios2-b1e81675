-- Resolver for the course_info_v1 WhatsApp template.
--
-- Given a lead_id, returns the four body params + two button URLs the
-- template needs. Edge function whatsapp-send calls this when invoked with
-- template_key=course_info_v1 and no explicit params, so the same template
-- works for AI post-call, manual disposition nudges, and ad-hoc sends
-- without each caller hand-assembling the message.
--
-- Output jsonb shape (NULL when the lead has no course_id):
--   {
--     "student_name": "Rahul Sharma",
--     "course_name":  "Bachelor of Science in Nursing",
--     "duration":     "4 years (semester)",
--     "eligibility":  "10+2 PCB, min 50%, no entrance exam",
--     "approval":     "INC, AKTU",
--     "video_url":    "https://youtu.be/...",
--     "course_url":   "https://www.nimt.ac.in/courses/bachelor-of-science-in-nursing#admissions"
--   }
...
BEGIN
  ...
  SELECT c.id, c.name, c.code, c.duration_years, c.type, c.video_url, c.slug, c.curriculum_url
    INTO v_course
  ...
  -- Duration string: "4 years (semester)"
  v_duration := v_course.duration_years
                || ' year' || CASE WHEN v_course.duration_years = 1 THEN '' ELSE 's' END
                || ' (' || v_course.type || ')';

  -- Eligibility — prefer the curated notes, else compose from structured fields.
  SELECT
    COALESCE(
...
REVOKE ALL ON FUNCTION public.fn_resolve_course_info_params(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_resolve_course_info_params(uuid) TO authenticated, service_role;

COMMENT ON FUNCTION public.fn_resolve_course_info_params(uuid) IS
  'Builds WhatsApp course_info_v1 template params for a lead by joining courses + eligibility_rules + approval_letter_courses. Returns NULL when the lead has no course_id; caller should fall back to course_info_generic.';
