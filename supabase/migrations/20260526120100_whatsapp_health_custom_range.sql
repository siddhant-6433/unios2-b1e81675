-- Extend fn_whatsapp_health_dashboard with an optional custom date range so
-- the WhatsApp Health UI can offer Today / Yesterday / Custom alongside the
-- existing N-day window presets.
--
-- When p_from is provided, the window is [p_from, COALESCE(p_to, now())] and
-- p_days is ignored. window_days in the response is computed from that span
-- (ceil), so the rest of the dashboard keeps showing a coherent label.

DROP FUNCTION IF EXISTS public.fn_whatsapp_health_dashboard(integer);
...
BEGIN
  ...
  IF p_from IS NOT NULL THEN
    ...
    END IF;
    -- Guard against absurd ranges. 366 days is plenty.
...
COMMENT ON FUNCTION public.fn_whatsapp_health_dashboard(integer, timestamptz, timestamptz) IS
  'WhatsApp Health dashboard aggregator. Pass p_days for a rolling window, or p_from/p_to for a custom range (range overrides p_days). Restricted to super_admin / admission_head.';
