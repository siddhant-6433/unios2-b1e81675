...
async function buildLoanLetterPdf(opts: LoanLetterOpts): Promise<Uint8Array> {
  ...
  const row = (label: string, value: string, rowY: number, left = margin, w = width - margin * 2) => {
    page.drawRectangle({
      x: left, y: rowY - 25, width: w, height: 25,
    ...
    const valueLines = wrapText(value || "-", bold, 9, w * 0.58);
    page.drawText(valueLines[0] || "-", { x: left + w * 0.42, y: rowY - 16, size: 9, font: bold, color: rgb(0.10, 0.10, 0.14) });
  };
  ...
  page.drawText(`Date: ${fmtDate(new Date().toISOString())}`, {
    x: width - margin - 150, y: y + 2, size: 9, font, color: rgb(0.42, 0.42, 0.48),
  });
  y -= 34;
  ...
  feeTable();
  y -= 10;
  row("First-Year Applicable Fee", fmtINR(opts.firstYearFee), y);
  y -= 25;
  row("Total Programme Applicable Fee", fmtINR(opts.totalCourseFee || opts.offer?.total_fee || 0), y);
  y -= 25;
  row("Token Fee Required", fmtINR(opts.tokenRequired), y);
  y -= 25;
  row("Token Fee Paid", fmtINR(opts.tokenPaid), y);
  y -= 25;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if ((offer as any).loan_letter_url) {
    ...
    const path = `loan-letters/${offer.lead_id}/${offer.id}.pdf`;
