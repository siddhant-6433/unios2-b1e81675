      ...
      {loading ? <Loader /> : (
        <>
          <StatusCard
            ...
            tone="blue"
            onPress={() => router.push('/(family)/library' as any)}
          />
          ...
          <View style={styles.actionGrid}>
            <ActionTile icon={Barcode} label="Scan / digitize" subtitle="ISBN to review queue" tone="blue" onPress={() => router.push('/(family)/library' as any)} />
            <ActionTile icon={BookOpen} label="Issue book" subtitle="Admission number + accession" tone="green" onPress={() => router.push('/(family)/library' as any)} />
            <ActionTile icon={CheckCircle2} label="Return book" subtitle="Assess fines if overdue" tone="yellow" onPress={() => router.push('/(family)/library' as any)} />
            <ActionTile icon={Search} label="Catalog search" subtitle="Find copy availability" tone="purple" onPress={() => router.push('/(family)/library' as any)} />
          </View>
        </>
      )}
    </WorkScaffold>
  ...
  return (
    <WorkScaffold title="Teaching Work" subtitle={`${displayName}, your classes and students`}>
      <StatusCard
        ...
        value="0"
        tone="blue"
        onPress={() => router.push('/(staff)/work/classes' as any)}
      />
  ...
  return (
    <WorkScaffold title="My Work" subtitle={`${displayName}, quick access to work tools`}>
      ...
      <View style={styles.actionGrid}>
        ...
        <ActionTile icon={Users} label="Directory" subtitle="Find colleagues" tone="blue" onPress={() => router.push('/(staff)/work/team' as any)} />
      </View>
    </WorkScaffold>
  );
}
