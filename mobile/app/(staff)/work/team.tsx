...
export default function TeamScreen() {
  ...
  const fetchTeam = async () => {
    ...
    // Fetch all staff profiles; roles live in user_roles, not profiles.
    const [{ data: profiles }, { data: roleRows }] = await Promise.all([
      supabase
        .from('profiles')
        .select('user_id, display_name, phone, department, institution, campus')
        .order('display_name'),
    ...
    if (profiles) {
      ...
      // Staff only — exclude student/parent accounts.
      setEmployees(
        profiles
          .map((p) => ({ ...p, role: roleMap.get(p.user_id) ?? '' }))
          .filter((p) => p.role && !['student', 'parent'].includes(p.role)) as Employee[],
      );
    }
