...
import { useActiveChild } from '../../../features/family/ChildContext';
import { useChildFees } from '../../../api/family';
import { SegmentedControl } from '../../../components/ui';

export default function FeesScreen() {
  // Child resolution goes through ChildContext (own row for students,
  // father/mother/guardian-linked rows for parents — same as web ParentPortal).
  const { children_, activeChild, setActiveChildId } = useActiveChild();
  const feesQuery = useChildFees(activeChild?.id);

  const fees = (feesQuery.data?.items ?? []).map((item) => ({
    id: item.id,
    fee_head: item.feeName || item.term || 'Fee',
